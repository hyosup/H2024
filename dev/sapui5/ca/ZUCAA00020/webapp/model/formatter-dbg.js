sap.ui.define([
	...
	return {
		formatTreeIcon: function(sNodeType, bZdel) {
			if (bZdel) {
		...
		formatTreeTooltip: function(sNodeType) {
			switch (sNodeType) {
                case "expanded":
                    return "sap-icon://folder";
                case "leaf":
		...
		formatFileDeleteVisible:function(sValue){
			if(!this._userId){
				this._userId = sap.ushell.Container.getService("UserInfo").getUser().getId();
			}
			if(sValue === this._userId || this._isAuth){
				return true;
			}else{
				return false;
			}
